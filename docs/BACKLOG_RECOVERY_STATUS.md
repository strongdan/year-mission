...
- **YM-RM-003 — Sign in with Apple:** RETIRED by later owner decision. Apple OAuth was previously implemented and production-qualified, but the user-facing Apple login path is being removed. Apple Health / HealthKit remains a separate supported integration and is not affected.
...
The original recovery work was implemented and deployed through PR #34. Later owner decisions may retire individual recovered features; YM-RM-003 is now historical/retired rather than a current product requirement. Stale PR #24 has been closed.
