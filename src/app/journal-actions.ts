...
import { parseJournalAnalysis } from "@/domain/journal";
...
export async function analyzeJournalEntryAction(entryId: string) {
  ...
  try {
    ...
    const parsed = parseJournalAnalysis(reply.content);
    if (!parsed.analysis) return { ok: false as const, error: "AI returned no useful analysis." };
