  ...
  return (
    <div className="px-4 pb-4">
      <Card className="border-violet-950/70 bg-violet-950/10">
        ...
        {workoutStep !== null && <div className="mt-3 rounded-xl border border-violet-900/50 bg-zinc-950/30 p-3"><p className="text-xs text-violet-200">{SPEAKING_WORKOUT[workoutStep]} · {String(5 - Math.floor(seconds / 60)).padStart(2, "0")}:{String(60 - (seconds % 60)).padStart(2, "0")}</p><p className="mt-1 text-[11px] text-zinc-500">Pauses are allowed. No microphone or recording.</p></div>}
        ...
        <div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="ghost" onClick={() => void choose("repeat")} disabled={busy}>{view.path.status === "paused" ? "Resume this month" : "Repeat month"}</Button>{view.path.status !== "paused" && <><Button size="sm" variant="ghost" onClick={() => void choose("next")} disabled={busy}>Next month</Button><Button size="sm" variant="ghost" onClick={() => void choose("pause")} disabled={busy}><Pause className="mr-1 h-3.5 w-3.5" />Pause</Button></>}</div>
      </Card>
    </div>
  );
}
