...
const AI_PROVIDER_META: Record<UserAiProvider, { label: string; description: string }> = {
  ...
  groq: { label: "Groq", description: "Fast GPT-OSS models available under Groq's Free plan limits." },
...
export function SettingsView({ environment, buildSha }: { environment: string; buildSha: string }) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 pb-8">
      ...
      <Card>
        <CardHeader title="AI services & fallback" subtitle="Free-tier providers are tried before paid OpenAI unless you choose another first provider." right={<KeyRound className="h-4 w-4 text-violet-400" />} />
        {ai ? (
          <div className="flex flex-col gap-4">
            ...
            {ai.freeTier && !ai.mock && <p className="text-xs leading-relaxed text-emerald-400/80">The active first provider has a configured free-tier path. Provider limits still apply.</p>}
          </div>
