...
export function HypnosisPlayer({ taskId, onComplete }: { taskId?: string | null; onComplete?: () => void }) {
  ...
  async function finishSession() {
    ...
    setSaving(true);
    const durationSeconds = startedAt.current ? Math.max(30, Math.round((Date.now() - startedAt.current) / 1000)) : 30;
    ...
    <div className="flex flex-col gap-4 p-4 pb-8">
      ...
      {settings.hypnosisMedia.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-1">
          {settings.hypnosisMedia.map((item) => <button key={item.id} onClick={() => { setSelected(item); startedAt.current = null; }} className={`min-w-[150px] rounded-xl border px-3 py-2.5 text-left ${selected?.id === item.id ? "border-violet-700 bg-violet-950/25" : "border-zinc-800 bg-zinc-900/20"}`}><p className="truncate text-sm text-zinc-200">{item.title}</p><p className="mt-0.5 text-[11px] uppercase tracking-wide text-zinc-600">{item.type}</p></button>)}
        </div>
      )}
      ...
      {selected ? (
        <Card className="border-violet-900/50 bg-violet-950/10">
          ...
          {selected.type === "audio" && <audio className="mt-4 w-full" controls preload="metadata" src={selected.url} onPlay={beginListening} onEnded={() => void finishSession()} />}
          {selected.type === "video" && <video className="mt-4 aspect-video w-full rounded-xl bg-black" controls preload="metadata" src={selected.url} onPlay={beginListening} onEnded={() => void finishSession()} />}
          ...
          {!acknowledged && selected.type !== "external" && <p className="mt-3 text-xs text-amber-300">Confirm safe listening above before starting playback.</p>}
          {selected.type !== "external" && !acknowledged && <style>{`audio, video { pointer-events: none; opacity: .45; }`}</style>}
          <div className="mt-4 border-t border-zinc-800 pt-3"><Button size="sm" variant="secondary" onClick={() => void finishSession()} disabled={saving}>{saving ? "Saving…" : "End session"}</Button></div>
        </Card>
      ) : (
      ...
      {startedAt.current && <Button variant="secondary" onClick={() => void finishSession()} disabled={saving}>{saving ? "Saving…" : "Mark listening complete"}</Button>}
