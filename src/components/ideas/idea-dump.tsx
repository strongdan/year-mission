...
export function IdeaDump({ initialIdeas }: { initialIdeas: Idea[] }) {
  ...
  function toggleNarration() {
    ...
    if (!Recognition) {
      setError("Speech-to-text is not available in this browser. You can still type the thought here.");
      return;
    }
    ...
    const startingText = text.trimEnd();
    ...
    recognition.onresult = (event) => {
      ...
      const spacer = startingText && transcript ? " " : "";
      setText(`${startingText}${spacer}${transcript}`);
    };
    recognition.onerror = () => {
      setError("Narration stopped unexpectedly. Anything already transcribed is still in the box.");
      setListening(false);
      recognitionRef.current = null;
    };
    recognition.onend = () => {
    ...
    <div className="flex flex-col gap-4 p-4 pb-24">
      ...
      <Card className="border-violet-950/70 bg-violet-950/10">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <Button type="button" variant={listening ? "secondary" : "ghost"} onClick={toggleNarration} disabled={Boolean(busy)}>
            {listening ? <MicOff className="h-4 w-4" /> : <Mic className="h-4 w-4" />}
            {listening ? "Stop narration" : "Narrate"}
          </Button>
          <Button type="button" onClick={() => void save(false)} disabled={!text.trim() || Boolean(busy)}>{busy === "save" ? "Saving…" : "Save thought"}</Button>
          <Button type="button" variant="secondary" onClick={() => void save(true)} disabled={!text.trim() || Boolean(busy)}>
            <Sparkles className="h-4 w-4" />
            {busy === "save-organize" || busy === "organize" ? "Organizing…" : "Save & organize"}
          </Button>
        </div>
        {listening && <p className="mt-2 text-xs text-violet-300">Listening… speak naturally. The transcript stays editable before you save.</p>}
      </Card>
